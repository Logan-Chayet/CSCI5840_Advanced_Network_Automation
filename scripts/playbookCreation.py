...
def createAccess():
    ...
    with open('../ANSIBLE/roles/access/vars/main.yaml', 'w') as yaml_file:
...
def createCore():
    ...
    # Convert to YAML
    yaml_output = yaml.dump(config_data, default_flow_style=False)
    with open('../ANSIBLE/roles/core/vars/main.yaml', 'w') as yaml_file:
        yaml_file.write(yaml_output)
    return yaml_output
...
def createEdge():
    ...
    # Convert to YAML
    yaml_output = yaml.dump(config_data, default_flow_style=False)
    with open('../ANSIBLE/roles/edge/vars/main.yaml', 'w') as yaml_file:
        yaml_file.write(yaml_output)
    return yaml_output
...
def sendConfig():
    ...
    cfg_file = "../ANSIBLE/CFGS/"+hostname+".txt"
...
def sendConfig_ztp():
    ...
    cfg_file = "../golden_configs/ZTP_cfgs/"+hostname+".txt"
    with ConnectHandler(**device) as connection:
        connection.enable()
        output = connection.send_config_from_file(cfg_file)
    print(output)
